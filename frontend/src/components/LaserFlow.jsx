...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){
        ...
        float yp=(-R_V*uVLenFactor)*cos(tu),s=clamp(-yp/(R_V*uVLenFactor+EPS),0.0,1.0),spd=max(abs(sin(tu)),0.02);
        float env=pow(1.0-s,0.6)*spd;
        float cap=1.0-smoothstep(TOP_FADE_START,1.0,s); cap=pow(cap,TOP_FADE_EXP); env*=cap;
        float ph=s/max(FLOW_PERIOD,EPS)+uFlowTime*uFlowSpeed;
        float fl=pow(tri01(ph),FLOW_SHARPNESS);
        env*=mix(1.0-uFlowStrength,1.0,fl);
        float m=pow(smoothstep(FLARE_HEIGHT,0.0,yp),FLARE_EXP),wx=1.0+FLARE_AMOUNT*m;
        vec2 sig=vec2(wx,1.0),p=vec2(0.0,yp);
        float mask=step(0.0,yp);
        b+=wt*bsa(uvc,p,mask*env,sig);
    }
    float vLen=max(R_V*uVLenFactor,EPS);
    float sPix=clamp(yPix/vLen,0.0,1.0),topA=pow(1.0-smoothstep(TOP_FADE_START,1.0,sPix),TOP_FADE_EXP);
    float L=a+b*topA;
    ...
    float radialFade = 1.0 - smoothstep(0.0, 0.85, length(uvc) / (120.0 * max(uVLenFactor, 0.5)));
